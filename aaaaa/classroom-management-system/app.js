...
function processWorkbook(workbook) {
    ...
    for (const sheetName of workbook.SheetNames) {
        ...
        console.log(`尝试解析工作表: ${sheetName}`);
        console.log('数据样本:', jsonData.slice(0, 3));
        ...
        for (let i = 0; i < Math.min(10, jsonData.length); i++) {
            ...
            for (let j = 1; j < row.length; j++) {
                ...
                if (cell.match(/[一二三四五六日]\/(1-2|3-4|5-6|7-8)/)) {
                    headerRow = i;
                    hasTimeHeader = true;
                    break;
                }
        ...
        if (headerRow !== -1) {
            ...
            for (let j = 1; j < headers.length; j++) {
                ...
                const match = header.match(/([一二三四五六日])\/(1-2|3-4|5-6|7-8)/);
...
function displayClassrooms() {
    ...
    for (const classroom in classroomSchedule) {
        const schedule = classroomSchedule[classroom];
        ...
        if (hasData) {
            ...
            // 创建上午教室卡片
            if (hasMorningClass) {
                createClassroomCard(classroom, daySchedule, morningPeriods, morningContainer);
            }
            
            // 创建下午教室卡片
            if (hasAfternoonClass) {
                createClassroomCard(classroom, daySchedule, afternoonPeriods, afternoonContainer);
            }
            
            // 创建晚上教室卡片
            if (hasEveningClass) {
                createClassroomCard(classroom, daySchedule, eveningPeriods, eveningContainer);
            }
        }
    }
    ...
    if (morningClassrooms.size === 0) {
        morningContainer.innerHTML = `<p>上午(08:00-12:00)没有需要开放的教室</p>`;
    }
    
    if (afternoonClassrooms.size === 0) {
        afternoonContainer.innerHTML = `<p>下午(13:00-17:00)没有需要开放的教室</p>`;
    }
...
// 创建教室卡片
function createClassroomCard(classroom, schedule, periods, container) {
...
function handleReservation(event) {
    ...
    let timeDescription;
    if (timePeriod === 'morning') {
        timeDescription = '上午 (08:00-12:00)';
    } else if (timePeriod === 'afternoon') {
        timeDescription = '下午 (13:00-17:00)';
    } else if (timePeriod === 'evening') {
